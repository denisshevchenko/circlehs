...
API calls for work with info about projects' builds.
-}
...
--
-- main :: IO ()
-- main = runCircleCI (getFullBuildInfo (ProjectPoint "denisshevchenko" "circlehs")
--                                      BuildNumber 129)
...
-- @
getFullBuildInfo :: ProjectPoint                   -- ^ Names of GitHub user/project.
                 -> BuildNumber                    -- ^ Build number.
                 -> CircleCIResponse FullBuildInfo -- ^ Info about single build.
getFullBuildInfo project (BuildNumber aNumber) = do
    AccountAPIToken token <- ask
    liftIO . runExceptT $ do
        manager <- httpsManager
        servantGetFullBuildInfo (userName project)
                                (projectName project)
                                aNumber
                                (Just token)
                                manager
...
data FullBuildInfo = FullBuildInfo {
    gitHubUrl :: Text
    } deriving (Show)

instance FromJSON FullBuildInfo where
    parseJSON (Object o) = FullBuildInfo
        <$> o .: "vcs_url"
    parseJSON _ = mzero
...
-- Complete API for work with projects' builds.
type BuildInfoAPI = GetFullBuildInfoCall

-- Obtain full info about single build.
type GetFullBuildInfoCall =
       "project"
    :> Capture "username" UserName
    :> Capture "project" ProjectName
    :> Capture "build_num" Int
    :> QueryParam "circle-token" Token
    :> Get '[JSON] FullBuildInfo
...
servantGetFullBuildInfo :: UserName
                        -> ProjectName
                        -> Int
                        -> Maybe Token
                        -> Manager
                        -> BaseUrl
                        -> ClientM FullBuildInfo
servantGetFullBuildInfo = client buildInfoAPI

buildInfoAPI :: P.Proxy BuildInfoAPI
buildInfoAPI = P.Proxy
